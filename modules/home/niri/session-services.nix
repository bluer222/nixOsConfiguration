  ...
  systemd.user.services.keepassxc =
    let
      unlockScript = pkgs.writeShellScript "keepassxc-autounlock" ''
        ...
        # volume, same as the keyfile and everything else in $HOME. The store
        # is world-readable locally — acceptable for this threat model.
        printf '%s\n' 'dj2#9(jd@10' | ${pkgs.keepassxc}/bin/keepassxc --minimized \
